...
<body class="hold-transition skin-blue sidebar-mini">
							...
							<c:forEach items="${varList}" var="var" varStatus="vs">
								<tr>
									...
									<td style="width: 180px;text-align: center;">
										<button type="button" class="btn bg-purple btn-xs btn-flat" onclick="relatedAnchor('${var.id}')">关联的基站</button>
										<button type="button" class="btn bg-purple btn-xs btn-flat" onclick="relatedClock('${var.id}')">关联的时钟源</button>
			...
			<table style="width:100%;">
				<tr>
					<td style="vertical-align:top;">
						<a class="btn btn-small btn-success btn-sm btn-flat" name="addArea" onclick="addArea();">添加区域</a>
					</td>
	...
	<script type="text/javascript">
		...
		function submitAreaForm(){
			...
			$.ajax({
				type:'POST',  
			    url:'${basePath}/need_show_area_range/save.do?idsList='+idsList, 
			    cache: false,
		...
		function del(Id){
			Dialog.confirm("确定要从列表中移除吗?", function(){
				...
				$.ajax({
					type: "POST",
					url: "${basePath}/need_show_area_range/remove.do?areaId="+Id+"&tm="+new Date().getTime()+'&areaIds='+"${pd.areaIds}",
					dataType:'json',
					cache: false,
					success: function(data){
						hideLoading(); 
						if(data.status=="ok")
							{
							window.location = '${basePath}/need_show_area_range/needToShowAreaList.do?idsList='+data.idsList;
				     		}
					}
				});
			});
		...
		//关联的基站列表
		function relatedAnchor(id){
			showDialog = new Dialog();
			showDialog.Width = 1000;
			showDialog.Height = 800;
			showDialog.Title = "该区域关联的基站";
			showDialog.URL = '${basePath}/need_show_area_range/relatedAnhor.do?areaId='+id+"&flag=anchor";
		...
		//关联的时钟源
		function relatedClock(id){
			showDialog = new Dialog();
			showDialog.Width = 1000;
			showDialog.Height = 800;
			showDialog.Title = "该区域关联的基站";
			showDialog.URL = '${basePath}/need_show_area_range/relatedAnhor.do?areaId='+id+"&flag=clock";
			showDialog.OKEvent = function(){
				showDialog.innerFrame.contentWindow.save();
			};
			showDialog.CancelEvent = function(){//关闭事件
