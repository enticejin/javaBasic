...
<body class="hold-transition skin-blue sidebar-mini">
	<div class="container-fluid" style="padding:10px 23px 10px 23px;">
		<!-- 检索  -->
		<form action="needToShowClockList.do" method="post" name="Form" id="Form">
			<div class="row table-query">
				<table>
					<tr>
						<td><input type="hidden" id="clockId" name="clockId" value="${pd.clockId}"></td>
						...
						<c:when test="${not empty varList}">
							<c:forEach items="${varList}" var="var" varStatus="vs">
								...
								<%-- 	<td style="width: 180px;text-align: center;">
										...
										<button type="button" class="btn bg-purple btn-xs btn-flat" onclick="cancelRelated('${var.id}')">取消关联</button>
	...
	<script type="text/javascript">
		...
		//取消与区域关联
		function cancelRelated(id){
			Dialog.confirm("确定要取消关联吗?", function(){
				showLoading();//显示遮罩层
				$.ajax({
					type: "get",
					url: "${basePath}/need_show_clock_rang/cancelRelated.do?areaId="+id+"&tm="+new Date().getTime(),
					dataType:'json',
					cache: false,
					success: function(data){
						...
						if(data.status=="ok")
							{
							//加载取消关联区域后的列表
							window.location = '${basePath}/need_show_clock_rang/related.do?clockId='+data.clockSourceId+'&areaIdList='+data.areaIdList;
							}else
								{
									Dialog.alert(data);	
								}	
		...
		//异步提交表单
		function submitClockAreaForm()
		{
			var idsList ="${pd.areaIdList}";
			var mydata = $("#Form").serialize();  
			$.ajax({
				type:'POST',  
			    url:'${basePath}/need_show_clock_rang/save.do?areaIdList='+"${pd.areaIdList}"+'&flag='+'area', 
			    cache: false,
