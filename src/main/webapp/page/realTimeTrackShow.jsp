...
<head>
  <meta charset="utf-8">
	...
	<link rel="stylesheet" href="${basePath}/staticres/test/test.css" />
<style>
.box-body {
	padding: 5px;
}
 .ol-rotate{
  ...
  <input type="hidden" name="FLOORORG_ID" id="FLOORORG_ID" value="${floor.id}"/>
  <div style="padding:10px;">
	        	...
	        	<div class="box box-primary" >
		            ...
		            <div class="box-body" >
		            	<div id="mapDiv"   style="border:1px solid #E4E5E6;float:left;width:100%;height:100%;position: absolute;">
		              		<div id="floorLayers" class="floor-list-box" style="border:0px solid #ccc;border-radius: 3px;width:50px;height:100px;background-color:rgba(255,255,255,.9);position:absolute;right:3px;bottom:100px;z-index:1000">
		              			<div class="amap-indoormap-floorbar-control can-scroll can-scroll-up" style="display: block;"  data-spm-anchor-id="0.0.0.i7.42e51c4eEjYC1Q">
									<div class="panel-box">
										<div class="select-dock"></div>
										<!-- <div class="floor-btn floor-nav floor-plus" onclick="switchPreFloor()">+</div> -->
										<div class="floor-list-box">
											<ul class="floor-list" >
												<!-- 在此循环将楼层加载出来 -->
												<c:forEach items="${allFloorList}" var="var" varStatus="vs">
													<li class="floor-list-item" title="${var.name}" onclick="handleFloor('${vs.index}','${var.id}')"><div class="floor-btn floor-nonas" id="floorName">${var.name}</div></li>
												</c:forEach>
											</ul>
										</div>
										<!-- <div class="floor-btn floor-nav floor-minus" onclick="switchNextFloor()">-</div> -->
									</div>
								</div>
		              		</div>
		              ...
		              <%-- 	<div id="mapDiv"   style="border:1px solid #E4E5E6;float:left;width:100%;height:100%;position: absolute;">
		              		<div id="map" style="width:100%;"></div>
		              		<div id="floorLayers" class="floor-list-box" style="border:0px solid #ccc;border-radius: 3px;width:50px;height:100px;background-color:rgba(255,255,255,.9);position:absolute;right:3px;bottom:100px;z-index:1000">
		              			<div class="amap-indoormap-floorbar-control can-scroll can-scroll-up" style="display: block;" title="" data-spm-anchor-id="0.0.0.i7.42e51c4eEjYC1Q">
									<div class="panel-box">
										<div class="select-dock"></div>
										<!-- <div class="floor-btn floor-nav floor-plus" onclick="switchPreFloor()">+</div> -->
										<div class="floor-list-box">
											<ul class="floor-list" >
												<!-- 在此循环将楼层加载出来 -->
												 <c:forEach items="${allFloorList}" var="var" varStatus="vs">
													  <li class="floor-list-item" title="${var.name}" onclick="handleFloor('${vs.index}','${var.id}')"><div class="floor-btn floor-nonas">${var.name}</div></li>
													 <li class="floor-list-item" title="${var.name}" onclick="handleFloor('${vs.index}','${var.id}')"><div class="floor-btn floor-nonas">${var.name}</div></li>
												</c:forEach>
	...
	$(function() {
		...
		currentFloorId=$("#FLOORORG_ID").val();
		getAreaIdList(currentFloorId);
		initCustomBar();
	});
	
	var currentFloorId //记录当前页面的楼层ID ，切换楼层的时候，应该改变
	var currentFloorOfAreaIDList=[] //记录当前楼层对应的区域列表
	//初始化
	function initCustomBar()
	{
		//debugger;
		//如果只有1层，则不显示这个按钮
		var eleCount=$(".floor-list").children().length;//楼层层数
		if(eleCount==1)
			{
				$("#floorLayers").css("display","none");
			}
		var currentTop=28*(eleCount-1);
		//$(".select-dock").css("margin-top",currentTop+"px");
	}
	function getAreaIdList(_myFloorId)
	{
		$.ajax({
			type : "get",
			url : '<%=basePath%>/rtlsConfig/json_areas',
			data : {floorId:_myFloorId},
			dataType : 'json',
			async : false,
				...
				if(data.features.length>0)
					{
						...
						{
							currentFloorOfAreaIDList.push(data.features[i].id);	
						}
					}
			}
		});
	}
	function handleFloor(index,_myFloorId)
	{
		//点击谁，就把框放到谁的上面
		var eleCount=$(".floor-list").children().length;//楼层层数
		var currentTop=28*index;
		$(".select-dock").css("margin-top",currentTop+"px");
		//先根据楼层ID获取该楼层有哪些区域列表
		currentFloorOfAreaIDList=[];
		$.ajax({
			type : "get",
			url : '${basePath}/rtlsConfig/json_areas',
			data : {floorId:_myFloorId},
			...
			success : function(data) {
				...
				getTargetLayer(_myFloorId);
			}
		});
	}
	
	function getTargetLayer(_myFloorId)
	{
		//debugger;
		var test=map.getLayers()[0];
		var testsource=map.getLayers().getArray()[1];//先找地图层
		var layerCount=map.getLayers().getLength();
		//删除楼层图，删除区域，删除基站
		var delLayer=[];
		for(var i=0;i<layerCount;i++)
		{
			var floorLayer=map.getLayers().getArray()[i];
			
			if(floorLayer)
			{
				
				var pcnName=floorLayer.getProperties().title;
				//异步判断楼层名称
				$.ajax({
					type : "get",
					url : '${basePath}/rtlsConfig/getFloorName.do',
					//data : {floorId:_myFloorId},
					dataType : 'json',
					async : false,
					success : function(data) {
						if(data.floorNames.length > 0){
							for(var i = 0;i < data.floorNames.length;i++){
								if(pcnName.trim() === data.floorNames[i].trim()){
									delLayer.push(floorLayer);
								}
							}
						}
					}
		...
		{
			console.info("删除"+delLayer[i].getProperties().title)
			map.removeLayer(delLayer[i]);
		}
		//加载平面图
			loadFloorMap(_myFloorId);
		//加载区域
			loadAreaToMap(_myFloorId);
		//加载基站
			loadAnchorToMap(_myFloorId);
		//设置显示坐标的层在最上面
			setCoordLayerToTop();
		
	}
	
	function setCoordLayerToTop()
	{
		var coordlayIndex=[];
		var coordlay=[];
		var layerCount=map.getLayers().getLength();
		for(var i=0;i<layerCount;i++)
		{
			...
			if(floorLayer)
			{
				...
				if(pcnName=="轨迹层")
					{
						coordlay.push(floorLayer);
						coordlayIndex.push(i);
					}
			}
			
		}
		//设置顺序
		var allMapLays=map.getLayers();
		for(var i=0;i<coordlayIndex.length;i++)
		{
			allMapLays.removeAt(coordlayIndex[i]);//先删除
		}
		for(var i=0;i<coordlay.length;i++)
		{
			map.addLayer(coordlay[i]);//添加
		}
	}
	//异步加载区域数据
	function loadAreaToMap(_myFloorId)
	{
		var myAreasLayer = new ol.layer.Vector({
			title: "区域层",    		
	    	source: new ol.source.Vector({
	    			url: '../rtlsConfig/json_areas?floorId='+_myFloorId,
	      			format: new ol.format.GeoJSON()
	    		}),
	    	style: areasStyleFunction
		});
		map.addLayer(myAreasLayer);
	}
	//异步加载基站数据
	function loadAnchorToMap(_myFloorId)
	{
		var myAnchorsLayer = new ol.layer.Vector({
			title: "基站层",    		
			source: new ol.source.Vector({
					url:'../rtlsConfig/json_anchors_byFloorId?floorId='+_myFloorId,
					
    				format: new ol.format.GeoJSON()
  				}),
  			style: anchorStyleFunction
		});
		map.addLayer(myAnchorsLayer);
	}
	//异步获取楼层平面图
	function loadFloorMap(_myFloorId)
	{
		
		$.ajax({
			type: "get",
			url: '${basePath}/rtlsConfig/getJsonByFloorId.do',
			data : {floorId:_myFloorId},
			dataType:'json',
			async:false,
			success: function(data){
				...
				if(data.length==0)
				{
					Dialog.alert("该楼层并未设置平面图，无法加载平面图");
					return;
				}
				for(var i=0;i<data.length;i++)
				{
					...
					if(_myFloorId === data[i].floor.id){
						var planMapImg=new Image();
						planMapImg.src=data[i].ufstring;
						 	var planMapLayer=new ol.layer.Image({
							name : data[i].floor.name,//data[i].floor.name,
					...
					map.addLayer(planMapLayer);//添加到图层中
				}
			 }
		});
	}
	...
	var myModify = new Modify({
		  features: mySelect.getFeatures()
		}); */
	
	//默认最多显示5层，超过5层后就显示，上下切换按钮
	function checkDownUp()
	{
		
	}
	
	function switchFunc(n_value)
	{
		alert(n_value)
	}
	//下一层
	function switchNextFloor()
	{
		var oldTop=$(".floor-list").css("margin-top").replace("px","");
		var currentTop=parseInt(oldTop) - 28;
		$(".floor-list").css("margin-top",currentTop+"px");
	}
	//上一层
	function switchPreFloor()
	{
		var oldTop=$(".floor-list").css("margin-top").replace("px","");
		var currentTop=parseInt(oldTop) + 28;
		$(".floor-list").css("margin-top",currentTop+"px");
			...
			success: function(data){
				...
				for(var i=0;i<data.length;i++)
				{
					...
					var palnMapLayer=new ol.layer.Image({
						name : data[i].floor.name,//data[i].floor.name,
						title : data[i].floor.name,//data[i].floor.name,
						opacity : 1,
						source : new ol.source.GeoImage({
							image :planMapImg,
							imageCenter : ol.proj
