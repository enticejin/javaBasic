...
<body class="hold-transition skin-blue sidebar-mini">
	<div class="container-fluid" style="padding:10px 23px 10px 23px;">
		<!-- 检索  -->
		<form action="needToShowTagList.do?idsList=${pd.idsList}" method="post" name="Form" id="Form">
		<div class="row table-query">
				<table>
					<tr>
						<td><input type="hidden" name="ids" id="ids" value="${pd.ids}"></td>
						<td><input class="form-control input-sm"  id="nav-search-input" type="text" id="tagName" name="tagname"  placeholder="标签名称" /></td>
					...
					<c:choose>
						<c:when test="${not empty varList}">
							<c:forEach items="${varList}" var="var" varStatus="vs">
								<tr>
									<td><input type='checkbox' name='answer' value="${var.id}" /></td>
											<td>${var.id}</td>
											<td>${var.name}</td>
											<td>${var.comments}</td>  
									<td style="width: 180px;text-align: center;">
										...
										<a class="btn btn-social-icon btn-danger btn-xs btn-flat" title="从列表中移除" onclick="del('${var.id}');"><i class="fa fa-trash"></i></a>
									</td>
								</tr>
							</c:forEach>
						</c:when>
			...
			<table style="width:100%;">
				...
				<td style="vertical-align:top;">
						<a class="btn btn-small btn-success btn-sm btn-flat" name="addTagList" id="addTag" onclick="addTagList()">添加标签</a>
					</td>
	...
	<script type="text/javascript">
		...
		function del(Id){
			Dialog.confirm("确定要从列表中移除吗?", function(){
				...
				$.ajax({
					type: "POST",
					url: "${basePath}/need_show_rang/remove.do?tagId="+Id+"&tm="+new Date().getTime()+'&ids='+"${pd.ids}",
					dataType:'json',
					cache: false,
					success: function(data){
						...
						if(data.status=="ok")
							{
							window.location = '${basePath}/need_show_rang/needToShowTagList.do?idsList='+data.idsList;
				     		}
					}
				});
			});
		...
		function submitTagForm()
		{
			...
			$.ajax({
				type:'POST',  
			    url:'${basePath}/need_show_rang/save.do?idsList='+idsList, 
			    cache: false,
