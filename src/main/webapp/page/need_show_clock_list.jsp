...
<body class="hold-transition skin-blue sidebar-mini">
	<input type="hidden" value="${pd.areaIdList}" id="areaIdList">
	<div class="container-fluid" style="padding:10px 23px 10px 23px;">
		<!-- 检索  -->
		<form action="cslist.do" method="post" name="Form" id="Form">
			<div class="row table-query">
				<table>
					<tr>
						<td><input class="form-control input-sm"  id="nav-search-input" type="text" name="NAME" value="${pd.NAME }" placeholder="时钟源名称" /></td>
							...
							<c:forEach items="${varList}" var="var" varStatus="vs">
								<tr>
									...
									<td style="width: 180px;text-align: center;">
										...
										<button type="button" class="btn bg-purple btn-xs btn-flat" onclick="selectArea('${var.id }');">关联的区域</button>
			...
			<table style="width:100%;">
				...
				<td style="vertical-align:top;">
						<a class="btn btn-small btn-success btn-sm btn-flat" name="ClockSourceList" id="ClockSource" onclick="addClockSourceList()">添加时钟源</a>
					</td>
	...
	<script type="text/javascript">
		...
		//修改坐标
		function setXYZ(id){
			var clockSouceIds = "${pd.ids}"
			var diag = new Dialog();
			diag.Title = "修改坐标";
			diag.URL = "setXYZ.do?clockSouceIds="+ clockSouceIds;
			diag.Width = 300;
			diag.Height = 180;
			diag.OKEvent = function(){
				...
				//提交表单
				$.ajax({
					url : 'setXYZ.do?clockSouceIds='+ clockSouceIds,
					type : 'POST',
					data : {
						'id': id,
					...
					success : function(data) {
						if(data.status=="ok"){
							diag.close();
							window.location = '${basePath}/need_show_clock_rang/needToShowClockList.do?idsList='+data.clockSouceIds;
						}else{
							Dialog.alert("设置失败");
						}
					},
		...
		//移除
		function del(Id){
			Dialog.confirm("确定要从列表中移除吗?", function(){
				showLoading();//显示遮罩层
				$.ajax({
					type: "POST",
					url: "${basePath}/need_show_clock_rang/remove.do?clockId="+Id+"&tm="+new Date().getTime()+'&ids='+"${pd.ids}",
					dataType:'json',
					cache: false,
					success: function(data){
						hideLoading(); 
						if(data.status=="ok")
							{
							window.location = '${basePath}/need_show_clock_rang/needToShowClockList.do?idsList='+data.idsList;
				     		}
					}
				});
			});
			...
			var idsList = [];
			  $('input:checkbox').each(function(){
		        ...
		        if(idsList.length == 0){
					...
					showDialog.Height = 400;
					showDialog.Title = "需要显示测距消息的时钟列表";
					showDialog.URL = '${basePath}/need_show_clock_rang/needToShowClockList.do?idsList='+idsList;
		...
		function selectArea(id){
			$.ajax({
				type: "POST",
				url: "${basePath}/need_show_clock_rang/related.do?clockId="+id+"&flag="+'${pd.flag}',
				dataType:'json',
				cache: false,
				success: function(data){
					debugger;
			...
			showDialog.Width = 800;
			showDialog.Height = 400;
			showDialog.Title = "该时钟关联的区域";
			showDialog.URL = '${basePath}/need_show_clock_rang/related.do?clockId='+id+"&flag="+'${pd.flag}';
			showDialog.OKEvent = function(){
				showDialog.innerFrame.contentWindow.save();
			};
			showDialog.CancelEvent = function(){//关闭事件
		...
		//保存
		function save(){
			submitClockSourceForm();
		}
		//异步提交表单
		function submitClockSourceForm(){
			var idsList="${pd.ids}";
			var mydata = $("#Form").serialize();  
			$.ajax({
				type:'POST',  
			    url:'${basePath}/need_show_clock_rang/save.do?idsList='+idsList, 
			    cache: false,
		...
		//从时钟标签列表添加需要显示测距消息的时钟源
		function addClockSourceList(){
