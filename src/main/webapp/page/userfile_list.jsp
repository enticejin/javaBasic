...
<body class="hold-transition skin-blue sidebar-mini">
	<div class="container-fluid" style="padding:10px 23px 10px 23px;">
		<!-- 检索  -->
		<form action="userFileList.do" method="post" name="Form" id="Form">
			<div class="row table-query">
				<table>
					<tr>
						<td><input class="form-control input-sm"  id="nav-search-input" type="text" name="fileName" value="${pd.fileName }" placeholder="文件名称" /></td>
					...
					<c:choose>
						<c:when test="${not empty varList}">
							<c:forEach items="${varList}" var="var" varStatus="vs">
								<tr>
									<td><input type='checkbox' name='ids' value="${var.fileName}" /></td>
									<td>${vs.index+1}</td>
											<td>${var.fileName}</td>
											<td>${var.size}</td>
											
									<td style="width: 100px;text-align: center;">
										<a class="btn btn-social-icon btn-danger btn-xs btn-flat" title="删除" onclick="delPicture('${var.fileName}');"><i class="fa fa-trash"></i></a>
									</td>
								</tr>
							</c:forEach>
						</c:when>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					<a class="btn btn-small btn-success btn-sm btn-flat" onclick="uploadWin()">上传新文件</a>
	...
	<script type="text/javascript">
		...
		//删除
		function delPicture(_fileName){
			Dialog.confirm("确定要删除吗?", function(){
				showLoading();//显示遮罩层
				$.ajax({
					type: "POST",
					url: "${basePath}/userFile/delPicture.do?fileName="+_fileName+"&tm="+new Date().getTime(),//删除操作地址
					dataType:'text',
					cache: false,
					//执行后台数据
		...
		var diagupload;
		function uploadWin(){
			diagupload = new Dialog();
			diagupload.Width = 770;
			diagupload.Height = 450;
			diagupload.Title = "文件上传";
			diagupload.URL = "${basePath}/userFile/uploadpage.do?allowedFileExtensions=['BMP', 'JPG','JPEG','PNG','GIF']&maxFileCount=1&theme='fa'";
			
			diagupload.OKEvent = function(){
				diagupload.close();
				query();
			}
			diagupload.show(); 
