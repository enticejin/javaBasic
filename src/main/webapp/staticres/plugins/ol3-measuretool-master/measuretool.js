...
ol.control.MeasureTool = function(opt_options) {
  ...
  var html = '';
  html += '<ul id="ulbodyBtn" class="ulbody">';
  html += '<li><input type="button" value="Line"></li>';
  html += '<li><input type="hidden" value="Area"></li>';
  html += '<li><input type="hidden" value="no"></li>';
  html += '</ul>';
...
ol.control.MeasureTool.prototype.mapmeasure = function(typeSelect) {
  ...
  function createMeasureTooltip() {
    ...
    index ++;
    measureTooltipElement.id = "top" + index;
  ...
  popupcloser.onclick = function(e) {
    ...
    var fs = vector.getSource().getFeatures();
    for(var i=0;i<fs.length;i++){
    	if(fs[i].getId() == eId){
    		vector.getSource().removeFeature(fs[i]);
    	}
    }
    
    var doc = document.getElementById("top"+eId);
