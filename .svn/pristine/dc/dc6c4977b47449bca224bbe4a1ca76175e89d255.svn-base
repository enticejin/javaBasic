...
public class Page {
	...
	private  static final String PAGE	= "config/PAGE.txt";
	...
	public Page(){
		try {
			this.showCount =12; //Integer.parseInt(Tools.readTxtFile(PAGE));
		} catch (Exception e) {
			this.showCount = 12;
		}
	}
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			sb.append("	<ul class=\"pagination btn-sm btn-flat\">\n");
			if(currentPage==1){
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage\" style=\"width:50px;height:33px;text-align:center;float:left\" placeholder=\"页码\"/></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"toTZ();\"  class=\"btn btn-sm btn-flat\">跳转</a></li>\n");
			...
			sb.append("	<li><select class=\"form-control input-sm\" title='显示条数' style=\"width:66px;height:33px;float:left;margin-left:-1px;\" onchange=\"changeCount(this.value)\">\n");
			...
			sb.append("  window.parent.showLoading();\n");
	...
	public String getPageStr1() {
		StringBuffer sb = new StringBuffer();
		if(totalResult>0){
			sb.append("	<ul class=\"pagination btn-sm btn-flat\" style=\"margin: 0px 0;display:block\">\n");
			int showTag = 5;//分页标签显示数量
			int startTag = 1;
			if(currentPage>showTag){
				startTag = currentPage-1;
