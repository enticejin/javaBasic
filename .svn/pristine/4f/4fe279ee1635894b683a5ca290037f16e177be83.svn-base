			...
			<table id="table_report"
				...
				<tr>
					...
					<td><input type="text" class="form-control input-sm" id="name"
						name="name" value="${name}" placeholder="这里输入区域名称"
						title="区域名称"></td>
...
/* 			var name = $("#name").val();
			var name1 = name.substring(name.length-3,name.length-1);
			if(name1 == "副本"){
				var num = parseInt(name.substring(name.length-1))
				num++;
				name = name.substring(0,name.length-2)+num;
				name.innerHTML=name;
			} */
