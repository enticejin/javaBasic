...
<body class="hold-transition skin-blue sidebar-mini">
	...
	<div class="container-fluid" style="padding:10px 23px 10px 23px;">
		<!-- 检索  -->
		<form action="tagTypeList.do" method="post" name="Form" id="Form">
			<%-- <div class="row table-query">
				<table>
					<tr>
						<td><input class="form-control input-sm"  id="nav-search-input" type="text" name="tagId" value="${pd.id }" placeholder="标签ID" /></td>
						...
						<c:when test="${not empty varList}">
							<c:forEach items="${varList}" var="var" varStatus="vs">
								<tr>		
											...
											<!-- <td id="tagName" name="tagName"><span class="typeClass"></span></td> -->
											
											<td> ${var.id }</td>
											<td>${var.name}</td>
											<td>${var.comments}</td>
											<td>${var.defaultIcon}</td>
											<!-- <td id="tagIcon"><span class="iconClass"></span></td> -->
 											<%-- <td>${var.icon}</td> --%>
										<td style="width: 180px; text-align: center;">
											<!-- 修改标签配置信息 --> <a title="修改"
											class="btn btn-social-icon  btn-primary btn-xs btn-flat"
											onclick="edit('${var.id}');"><i class="fa fa-edit"></i></a> <!-- 删除标签配置信息 -->
											<a class="btn btn-social-icon btn-danger btn-xs btn-flat"
											title="删除" onclick="del('${var.id}');"><i
			...
			<table style="width:100%;">
				<tr>
					<td style="vertical-align:top;">
						<a class="btn btn-small btn-success btn-sm btn-flat" name="defaultIcon" onclick="add();">新增标签类型</a>
					</td>
	...
	<script type="text/javascript">
		...
		function edit(id){
			...
			editDiag.Height = 420;
			editDiag.Title = "编辑";
			editDiag.URL = '${basePath}/tagType/tagEdit.do?tagId='+id;
		...
		var addDiag;
		function add(){
			var defaultIcon="${varList[0].defaultIcon}";
			addDiag = new Dialog();
			addDiag.Width = 770;
			addDiag.Height = 420;
			addDiag.Title = "新增";
			//addDiag.URL = "${basePath}/tagType/tagAdd.do?tagId="+id;
			addDiag.URL = '${basePath}/tagType/tagAdd.do?defaultIcon='+defaultIcon;
		...
		function del(Id){
			Dialog.confirm("确定要删除吗?", function(){
				...
				$.ajax({
					type: "POST",
					url: "${basePath}/tagType/delete.do?tagId="+Id+"&tm="+new Date().getTime(),
					dataType:'json',
					cache: false,
