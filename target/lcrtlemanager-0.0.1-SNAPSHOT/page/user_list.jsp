...
<body class="hold-transition skin-blue sidebar-mini">
	<div class="container-fluid" style="padding:10px 23px 10px 23px;">
		<!-- 检索  -->
		<form action="userList.do" method="post" name="Form" id="Form">
			<div class="row table-query">
				<table>
					<tr>
						<td><input class="form-control input-sm"  id="nav-search-input" type="text" name="NAME" value="${pd.NAME }" placeholder="用户名" /></td>
					...
					<c:choose>
						<c:when test="${not empty varList}">
							<c:forEach items="${varList}" var="var" varStatus="vs">
								<tr>
									<td><input type='checkbox' name='ids' value="${var.name}" /></td>
									...
									<td style="width: 150px;text-align: center;">
										<a title="修改"  class="btn btn-social-icon  btn-primary btn-xs btn-flat" onclick="editUser('${var.name}');"><i class="fa fa-edit"></i></a>
										<a class="btn btn-social-icon btn-danger btn-xs btn-flat" title="删除" onclick="del('${var.name}');"><i class="fa fa-trash"></i></a>
									</td>
								</tr>
							</c:forEach>
						</c:when>
			...
			<table style="width:100%;">
				<tr>
					<td style="vertical-align:top;">
						<a class="btn btn-small btn-success btn-sm btn-flat" onclick="addUser();">新增用户</a>
	...
	<script type="text/javascript">
		...
		function addUser()
		{
			diagAddUser = new Dialog();
			diagAddUser.Width = 470;
			diagAddUser.Height = 210;
			diagAddUser.Title = "新增用户";
			diagAddUser.URL = "${basePath}/user/add.do";
			diagAddUser.OKEvent = function(){
				diagAddUser.innerFrame.contentWindow.save();//调用打开窗口中的保存方法
				
			};
			diagAddUser.CancelEvent = function(){ //关闭事件
				diagAddUser.close();
			}
			diagAddUser.show(); 	
		}
		...
		function editUser(_userName)
		{
			...
			diag.Height = 210;
			diag.Title ="编辑";
			diag.URL = '${basePath}/user/edit.do?userName='+_userName;
		...
		function del(userName){
			Dialog.confirm("确定要删除吗?", function(){
				...
				$.ajax({
					type: "POST",
					url: "${basePath}/user/delUser.do?userName="+userName+"&tm="+new Date().getTime(),
					dataType:'json',
					cache: false,
